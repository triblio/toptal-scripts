    ...
    (function(window) {
      function _getNamesFromIds(data){
        ...
        var lookup = {"personalizations":{},"campaigns":{},"contentHubs":{},"audiences":{}};
      ...
      function _postFirmographicData(){
        var data = {"name":"Triblio","sicCode":7372,"isIsp":false,"employees":"10 to 50","revenue":"$1,000,000 to $5,000,000","country":"United States","domain":"triblio.com","naicCode":511210,"employeesCode":2,"revenueCode":2,"region":"Virginia","subIndustry":"Software Publishers","industry":"Newspaper Publishers"};
console.log('Triblio initiated');var count = 0;function waitForGA(hadToAppendGAScript){ var wait = setInterval(function(){ var type = typeofGA(); if(type){ checkAndPostToGA(type); clearInterval(wait); } else if(count > 15 && !hadToAppendGAScript){  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){ (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o), m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m) })(window,document,'script','https://www.google-analytics.com/analytics.js','ga'); count = 0; clearInterval(wait); waitForGA(true); } else if(count > 15 && hadToAppendGAScript){ clearInterval(wait); } count++; }, 100);}function typeofGA(){ if(typeof ga === 'function'){ return "ga"; } else if(typeof __gaTracker === 'function'){ return "_gaTracker"; } else return null;}function checkAndPostToGA(type){ var trackingId = "UA-45657726-2";  if (type === "ga") { console.log('ga method found'); ga(function(tracker) { ga('create', trackingId, 'auto', 'triblio'); console.log(trackingId+' created'); ga('triblio.set', 'dimension1', data.name); ga('triblio.set', 'dimension2', data.revenue); ga('triblio.set', 'dimension3', data.naicCode); ga('triblio.set', 'dimension4', data.employees); ga('triblio.set', 'dimension5', data.domain); ga('triblio.send', 'pageview'); console.log('data posted for ' + data.name); console.log('Triblio finished'); }); }  else if (type === "_gaTracker") { __gaTracker('create', trackingId, 'auto', 'triblio'); console.log(trackingId+ ' created'); __gaTracker('triblio.set', 'dimension1', data.name); __gaTracker('triblio.set', 'dimension2', data.revenue); __gaTracker('triblio.set', 'dimension3', data.naicCode); __gaTracker('triblio.set', 'dimension4', data.employees); __gaTracker('triblio.set', 'dimension5', data.domain); __gaTracker('triblio.send', 'pageview'); console.log('data posted for ' + data.name); console.log('Triblio finished'); }}waitForGA();
      }
